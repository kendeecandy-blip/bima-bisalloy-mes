...
st.markdown("""
    ...
    .metric-card { background-color: #f8f9fa; border-left: 5px solid #003366; padding: 12px; border-radius: 5px; margin-bottom: 10px; }
...
def parse_tonnage_from_desc(desc):
    ...
    if match:
        ...
        else: return val * 150.0  # Asumsi default bobot per pcs 150kg
    return 1000.0
...
def init_db_data():
    ...
    # 1. Pastikan Vendor Internal Ada & Terapkan Validasi Filter Untuk Mencegah IntegrityError
    ...
    # 2. Baca file Excel manual untuk otomatisasi migrasi
    excel_file_path = 'data_manual_bima.xlsx'
    if os.path.exists(excel_file_path):
        try:
            df = pd.read_excel(excel_file_path, skiprows=3)
            for idx, row in df.iterrows():
                # ELIMINASI DATA KOSONG 'NaT - NaT' & PANDAS CLEAN ROW FIX
                if pd.isna(row.get('CUSTOMER')) or pd.isna(row.get('SO NO.')) or pd.isna(row.get('DESCRIPTION')):
                    continue
                ...
                # Filter baris rekapitulasi / sampah di bagian bawah Excel
                recap_keywords = ['TOTAL', 'OUTSTANDING', 'O', 'STEEL', 'FERRITIC']
                skip_row = False
                for kw in recap_keywords:
                    if kw == 'O':
                        if kw == raw_no or f" {kw} " in f" {raw_no} ": 
                            skip_row = True
                    else:
                        if kw in raw_no: 
                            skip_row = True
                ...
                if raw_cust.startswith("('") and raw_cust.endswith("',)"): 
                    raw_cust = eval(raw_cust)[0]
                ...
                # BUG FIX: Membersihkan format desimal bawaan Pandas pada nomor SO
                ...
                # REFAKTOR: Mengambil Nama Proyek Murni & Optimasi Tonnage
                if raw_desc.startswith("('") and raw_desc.endswith("',)"):
                    raw_desc = eval(raw_desc)[0]
                ...
                new_proj = Project(
                    ...
                    so_number=raw_so,
                    sales_pic="Sistem Migrasi",
                    total_tonnage=migrated_tonnage,
                    material_list=final_material,
                    is_jasa_only=False,
                    yej_cycle=yej_cycle_val,
                    start_date=(now_local - timedelta(days=30)).date(),
                    target_selesai_global=(now_local + timedelta(days=15)).date(),
                    status_proyek=proj_status_val,
                    logistic_status=logistic_status_val,
                    delivery_number=delivery_num_val if delivery_num_val else None
                )
                db.add(new_proj)
                db.commit()
                db.refresh(new_proj)
...
def display_logo():
    if os.path.exists("logo_bima.png"):
        st.image("logo_bima.png", width=250)
    else:
        st.markdown("<h2 style='color: #003366;'>🏗️ PT BIMA BISALLOY</h2>", unsafe_allow_html=True)
...
def view_registrasi_proyek():
    ...
    with st.expander("➕ Tambah Jenis Proses Fabrikasi Baru ke Master Data"):
        ...
        if st.button("Simpan ke Master Scope") and new_scope_input.strip():
            ...
            if not db.query(ScopeMaster).filter(ScopeMaster.name == clean_name).first():
                ...
                st.warning("Proses tersebut sudah ada di Master Data.")
    ...
    with st.form("register_project_form"):
        ...
        material_opts = [
            "FAB01 - Wear Plate", "FAB02 - Cladding", "FAB03 - Structural", 
            "FAB04 - Stainless Steel", "FAB05 - Roundbar", "FAB06 - Carbon Steel"
        ]
        ...
        if st.form_submit_button("Daftarkan Proyek & Lock Routing"):
            ...
            if not final_proj_name: st.error("❌ Nama Proyek wajib diisi!")
                ...
                if final_cust_id:
                    new_proj = Project(
                        customer_id=final_cust_id, project_name=final_proj_name, so_number=so_num,
                        sales_pic=pic, total_tonnage=float(total_tonnage), 
                        material_list=",".join(selected_materials), is_jasa_only=is_jasa_only_val,
                        yej_cycle=yej_cycle_val, status_proyek=ProjectStatus.WAITING_SCHEDULE
                    )
                    try:
                        db.add(new_proj)
                        db.commit()
...
def view_perencanaan_target():
    ...
    if not projects:
        st.info("Tidak ada proyek yang menunggu penjadwalan saat ini.")
    else:
        with st.form("target_form"):
            ...
            if st.form_submit_button("Set Target & Majukan ke 'Waiting Drawing'"):
                if target_date < start_date:
                    st.error("❌ Target selesai tidak boleh mendahului tanggal mulai!")
                else:
        ...
        with st.form("drawing_form"):
            ...
            if st.form_submit_button("Rilis ke Produksi"):
                if approved:
                    ...
                    st.warning("Centang kotak 'Approved' untuk merilis ke produksi.")
    db.close()

# --- MODULE 4: QC / PPIC (Update Progress Fisik) ---
def view_update_qc():
    st.header("⚡ Update Progress Fisik Lapangan (Tonnage Base)")
    db = SessionLocal()
    ...
    selected_project = st.selectbox(
        "Pilih Proyek Active", 
        projects, 
        format_func=lambda x: f"{x.so_number} - {x.customer.name} ({x.project_name})"
    )
    ...
    if not active_scopes:
        st.error("Proyek ini belum memiliki routing pekerjaan."); db.close(); return
    ...
    if selected_scope.name.upper() == "CUTTING":
        cutting_tech = st.selectbox(
            "Jenis Teknologi Cutting yang Digunakan (Sub-Scope)",
    ...
    selected_vendor = st.selectbox(
        "Vendor Pelaksana", 
        vendor_opts, 
        format_func=lambda x: x if isinstance(x, str) else f"{x.name} ({x.type.value})"
    )
    ...
    if selected_vendor == "[+ Ketik Vendor Baru...]":
        ...
        is_external = (selected_vendor.type == VendorType.EXTERNAL)
    
    st.markdown("---")
    
    # REFAKTOR: st.form membungkus st.file_uploader & Logika Over-Tonnage
    with st.form("qc_update_form"):
        st.markdown("**📸 Upload Foto Bukti Lapangan**")
        uploaded_file = st.file_uploader("Lampirkan Foto Bukti Visual Fisik (PNG, JPG, JPEG) - Akan Divalidasi saat Submit", type=["png", "jpg", "jpeg"])
        
        po_vendor_val = None
        if is_external:
            po_vendor_val = st.text_input("Nomor PO Vendor (Mandatory untuk Vendor External)")
        ...
        if st.form_submit_button("Submit Data Tonase Fisik"):
            # Perhitungan Validasi Over-Tonnage Khusus Status PASS
            all_logs_scope = db.query(ProjectProgressLog).filter(
                ProjectProgressLog.project_id == selected_project.id, 
                ProjectProgressLog.scope_id == selected_scope.id, 
                ProjectProgressLog.status == QCStatus.PASS
            ).all()
            ...
            if status_val == "Pass" and (total_scope_tonnage + tonase_hari_ini) > selected_project.total_tonnage:
            ...
            elif is_external and not po_vendor_val.strip():
                st.error("❌ Nomor PO Vendor WAJIB diisi untuk Vendor Eksternal!")
            elif selected_vendor == "[+ Ketik Vendor Baru...]" and not new_vendor_name.strip():
                st.error("❌ Nama Vendor Baru tidak boleh kosong!")
            elif status_val in ["Fail", "Hold"] and not notes.strip():
                st.error("❌ Catatan Temuan WAJIB diisi jika status Fail atau Hold!")
            else:
                final_vendor_id = None
                if selected_vendor == "[+ Ketik Vendor Baru...]":
                    existing_v = db.query(Vendor).filter(Vendor.name == new_vendor_name.strip()).first()
                    if existing_v:
                        final_vendor_id = existing_v.id
                    else:
                        try:
                ...
                new_log = ProjectProgressLog(
                    ...
                    po_vendor_number=po_vendor_val, log_date=now_local
                )
                db.add(new_log)
                db.commit()
                
                # Check Completion Progress Valid
...
# --- MODULE 4.5: UPDATE LOGISTIK & PENGIRIMAN (Fitur Baru) ---
def view_update_logistik():
    ...
    with st.form("logistic_form"):
        ...
        status_opts = [
            "🏭 FABRIKASI SELESAI (BELUM DIKIRIM)", 
            "🚚 DALAM PERJALANAN (BELUM DITERIMA)", 
            "✅ SELESAI TOTAL (SUDAH DITERIMA KONSUMEN)", 
            "Lainnya (Ketik Bebas)"
        ]
        
        col1, col2 = st.columns(2)
        default_index = 0
        if selected_proj.logistic_status in status_opts:
            default_index = status_opts.index(selected_proj.logistic_status)
        elif selected_proj.logistic_status:
            default_index = 3
...
def generate_excel_resume(project, logs):
    ...
    headers = ["Tanggal Log", "Tahapan (Scope)", "Vendor Pelaksana", "Tonase Aktual", "Status QC", "Catatan Temuan QC", "Foto Bukti"]
    ws.append([]) 
    ws.append(headers) 
...
def view_dashboard():
    ...
    st.markdown("---")
    st.subheader("📈 Analisis Kurva S & Unduh Laporan Excel")
    ...
    if active_projects:
        ...
        if start_date and end_date and end_date >= start_date:
            ...
            if proj_logs:
                # REFAKTOR: Left join S-Curve Anti-Kosong
                ...
                # Memastikan visual tidak blank dan index ter-mapping merata pada nilai nol
                df_chart['Akumulasi Aktual'] = 0.0
        ...
        st.info("Belum ada proyek berstatus 'In Production' atau 'Completed'.")
    db.close()
...
if not st.session_state["logged_in"]:
    ...
    with st.sidebar:
        ...
        menu = st.radio("Pilih Modul:", allowed_menus)
